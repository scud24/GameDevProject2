...
if(remove_target != pointer_null)
{
	with(remove_target)
	{
		...
		else if(child_link!=pointer_null)
		{
			child_link.parent_link = pointer_null;		
			scr_shiftLinksForward();
		}
		else
		{
			show_debug_message("Link " + string(id) + " had no connections to remove- parent: " + string(parent_link) + " child: " +string(child_link));
		}
