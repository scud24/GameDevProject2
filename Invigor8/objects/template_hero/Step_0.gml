...
if(current_stun_state != StunState.normal)
{
	...
	{
		if(last_direction >= 0) current_sprite_state = HeroState.knockbackStunRight;
		else current_sprite_state = HeroState.knockbackStunLeft;
	}
}
else if(current_action == Action.neutral && current_movement == Movement.idle)
{
	if(current_carry_item == pointer_null)
	{
...
if(!animation_in_progress)
{
	...
	switch(current_sprite_state)
	{
		...
		animation_in_progress = true;
		break;
		case HeroState.knockbackStunLeft:
		sprite_index = spr_hero_wallsplat;
		image_xscale = 1;
		break;
		case HeroState.knockbackStunRight:
		sprite_index = spr_hero_wallsplat;
		image_xscale = -1;
		break;
	}
