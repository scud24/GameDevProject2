...
if(current_formation == Formation.assemble)
{
	...
	{
		if(abs(jump_target.y-y) < 10 && jump_target.object_index!=obj_moving_platform_bounds_node&&jump_target.object_index!=obj_moving_platform_target_node)
		{
			move_type = "run";
		}
		else
		...
		if(jump_target.object_index==obj_moving_platform_bounds_node && !jump_target.node_active)
		{
			if(jump_started)
			{
				scr_getNextNode();
