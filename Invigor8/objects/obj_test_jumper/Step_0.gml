...
if(current_formation == Formation.assemble)
{
	...
	if(isGrounded && jump_started)
	{
		...
		scr_getNextNode();
		//show_debug_message("Character: " + string(id) + " Set target to next node: " + string(jump_target));
		jump_started = false;
	}
	if(jump_target != pointer_null)
	{
		if(abs(jump_target.y-y) < 10) // && jump_target.object_index!=obj_moving_platform_bounds_node &&(jump_target.object_index!=obj_moving_platform_target_node||jump_target!=current_node)
		{
			move_type = "run";
		}
		else
		...
		if(current_node != pointer_null && !current_node.node_active)
		{
			if(jump_started)
			{
				scr_getNextNode();
		...
		if(current_assemble_state != AssembleState.holding_for_node || (jump_target.id != current_node.id))
		{
			if(move_type = "jump")
			{
			...
			else if(move_type == "run")//run mode
			{
...
//show_debug_message("Movement at jumper: " + string(current_movement));
//-------------------------------------------------------------------------------
if (keyboard_check_pressed(vk_lcontrol)&&parent_link == pointer_null)
{
	scr_setPathParams();
}
