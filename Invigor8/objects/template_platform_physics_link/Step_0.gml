...
if(current_formation == Formation.holding_for_node)
{
...
if(current_stun_state == StunState.normal && current_formation!= Formation.holding_for_node)
{
...
{
	var nearest_platform = instance_nearest(x+local_hspeed,y,template_moving_platform);
	if(place_meeting(x+local_hspeed-nearest_platform.hspeed,y,template_moving_platform))
	{
		show_debug_message("ping x");
		while(!place_meeting(x+sign(local_hspeed)+nearest_platform.hspeed, y, template_moving_platform))
		{
			x+=sign(local_hspeed);
		}
		if(abs(local_hspeed) < abs(nearest_platform.hspeed))
		{
			local_hspeed +=nearest_platform.hspeed;
		}
	}
}*/
